...
7. **Pass summary results to VideoList** for per-video status display:
   ...
   - Add retry button on failed summary videos that calls `onRetrySummary(videoId)`
   - Wire retry with transcript access: In App.tsx, pass `transcriptResults` Map from `useTranscriptExtraction` alongside `getSummaryResult` to VideoList. In the `onRetrySummary` callback passed from App.tsx, lookup the transcript text via `transcriptResults.get(videoId)?.transcript`, then call `retryVideo(videoId, transcriptText)`. This keeps VideoCard simple (it just calls `onRetrySummary(videoId)`) while App.tsx handles the transcript lookup.
